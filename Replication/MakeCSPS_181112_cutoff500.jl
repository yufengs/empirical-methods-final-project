...
for t = 1:T
    ...
    for state = 1:S
        ...
        p_cut = 500                 # The very first 3.5'' HDDs in 1982 had the average price of $759.44/unit & could hold only 8.612MB, which translates into "price/GB" of [$759.44 / 8.612MB] x 1000 = 88.1839 x 1000 = 88,183.9 [$/GB]. Their 1982 shipment was 0.0015 million [= 1,500] units, which is practically zero.
...
matwrite("CS_cutoff500.mat", Dict(
    "CS_cutoff500" => CS_cutoff500
); compress = true)